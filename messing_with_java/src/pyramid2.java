...
public class pyramid2 {
    public static void main(String[] args) {
            ...
            int seedLength = String.valueOf((int) Math.pow(2,seed)-1).length();
            ...
            for (int row = 1; row <= seed; row++){
                ...
                for (int col = 0; col <= (seed*2)-2; col++){
                    if(row-col > 0) {
                        int fHalf = (int) Math.pow(2,col);


                        System.out.printf("%"+(-seedLength-1)+"s" , String.valueOf(fHalf));

                    }
                    else if(col < row*2-1){
                        int sHalf = (int) Math.pow(2,2*(row-1)-col);
                        System.out.printf("%"+(-seedLength-1)+"s" , String.valueOf(sHalf));
